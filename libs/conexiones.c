...
int suscribirse_a_cola(queue_name cola, char* ip, char* puerto) {
	...
	if (recv(socket_servidor, &respuesta_broker, sizeof(uint32_t), MSG_WAITALL)
			== -1)
		return -1;
...
uint32_t enviar_mensaje(char* ip, char* puerto, queue_name cola,
		void* estructura_mensaje, bool esperar_id) {
	...
	case APPEARED_POKEMON:
		;
		appeared_pokemon_msg* msg_appeared =
				(appeared_pokemon_msg*) estructura_mensaje;

		paquete->buffer->size = sizeof(uint32_t) * 3
				+ msg_appeared->tamanio_nombre;
		stream = malloc(paquete->buffer->size);

		memcpy(stream + offset, &(msg_appeared->tamanio_nombre),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(stream + offset, msg_appeared->nombre_pokemon,
				msg_appeared->tamanio_nombre);
		offset += msg_appeared->tamanio_nombre;
		memcpy(stream + offset, &(msg_appeared->coordenada_X),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
	...
	case GET_POKEMON:
		...
		offset += sizeof(uint32_t);
		memcpy(stream + offset, msg_get->nombre_pokemon,
				msg_get->tamanio_nombre);

		break;

	case LOCALIZED_POKEMON:
		;
		localized_pokemon_msg* msg_localized =
				(localized_pokemon_msg*) estructura_mensaje;

		uint32_t tamanio_pares_coordenadas = sizeof(uint32_t)
				* msg_localized->cantidad_posiciones * 2;

		paquete->buffer->size = sizeof(uint32_t) * 3 + tamanio_pares_coordenadas
				+ msg_localized->tamanio_nombre;
		stream = malloc(paquete->buffer->size);

		memcpy(stream + offset, &(msg_localized->id_correlativo),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(stream + offset, &(msg_localized->tamanio_nombre),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(stream + offset, msg_localized->nombre_pokemon,
				msg_localized->tamanio_nombre);
		offset += msg_localized->tamanio_nombre;
		memcpy(stream + offset, &(msg_localized->cantidad_posiciones),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(stream + offset, msg_localized->pares_coordenadas,
				tamanio_pares_coordenadas);
	...
	case CATCH_POKEMON:
		...
		paquete->buffer->size = sizeof(uint32_t) * 4
				+ msg_catch->tamanio_nombre;
		stream = malloc(paquete->buffer->size);

		memcpy(stream + offset, &(msg_catch->tamanio_nombre), sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(stream + offset, msg_catch->nombre_pokemon,
				msg_catch->tamanio_nombre);
	...
	case CAUGHT_POKEMON:
		;
		caught_pokemon_msg* msg_caught =
				(caught_pokemon_msg*) estructura_mensaje;

		paquete->buffer->size = sizeof(uint32_t) * 2;
		stream = malloc(paquete->buffer->size);

		memcpy(stream + offset, &(msg_caught->id_correlativo),
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
	...
	int total_bytes = paquete->buffer->size + sizeof(queue_name)
			+ sizeof(uint32_t);
	...
	uint32_t id;
	if (esperar_id) {
		int status_recv = recv(socket_receptor, &id, sizeof(uint32_t),
				MSG_WAITALL);
		if (status_recv == -1)
			id = -1;
	} else {
...
void* recibir_mensaje(queue_name cola, int socket) {
	...
	paquete->buffer = malloc(sizeof(t_buffer));
	if (recv(socket, &(paquete->buffer->size), sizeof(uint32_t), MSG_WAITALL)
			== -1) {
		free(paquete->buffer);
		free(paquete);
		return NULL;
	...
	if (recv(socket, paquete->buffer->stream, paquete->buffer->size,
			MSG_WAITALL) == -1) {
...
void* deserializar_buffer(queue_name cola, void* buffer_ptr) {
	...
	case NEW_POKEMON:
		...
		memcpy(msg_new->nombre_pokemon, stream + offset,
				msg_new->tamanio_nombre);
		offset += msg_new->tamanio_nombre;
	...
	case APPEARED_POKEMON:
		...
		memcpy(&(msg_appeared->tamanio_nombre), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		msg_appeared->nombre_pokemon = malloc(msg_appeared->tamanio_nombre);
		memcpy(msg_appeared->nombre_pokemon, stream + offset,
				msg_appeared->tamanio_nombre);
		offset += msg_appeared->tamanio_nombre;
		memcpy(&(msg_appeared->coordenada_X), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(&(msg_appeared->coordenada_Y), stream + offset,
				sizeof(uint32_t));
	...
	case GET_POKEMON:
		...
		memcpy(msg_get->nombre_pokemon, stream + offset,
				msg_get->tamanio_nombre);
	...
	case LOCALIZED_POKEMON:
		;
		localized_pokemon_msg* msg_localized = malloc(
				sizeof(localized_pokemon_msg));

		memcpy(&(msg_localized->id_correlativo), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		memcpy(&(msg_localized->tamanio_nombre), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		msg_localized->nombre_pokemon = malloc(msg_localized->tamanio_nombre);
		memcpy(msg_localized->nombre_pokemon, stream + offset,
				msg_localized->tamanio_nombre);
		offset += msg_localized->tamanio_nombre;
		memcpy(&(msg_localized->cantidad_posiciones), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
		...
		memcpy(msg_localized->pares_coordenadas, stream + offset,
				tamanio_pares_coordenadas);
	...
	case CATCH_POKEMON:
		...
		memcpy(msg_catch->nombre_pokemon, stream + offset,
				msg_catch->tamanio_nombre);
		offset += msg_catch->tamanio_nombre;
	...
	case CAUGHT_POKEMON:
		...
		memcpy(&(msg_caught->id_correlativo), stream + offset,
				sizeof(uint32_t));
		offset += sizeof(uint32_t);
...
// ------------------ Constructores
	...
	appeared_pokemon_msg* appeared_msg(char* nombre_pokemon, uint32_t x,
			uint32_t y) {
	...
	localized_pokemon_msg* localized_msg(uint32_t id_corr, char* nombre_pokemon,
			uint32_t cant_posiciones, uint32_t* pares) {
