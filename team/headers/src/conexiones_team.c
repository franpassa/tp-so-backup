...
void enviar_gets(t_list* objetivos_globales) {
	...
	void enviar(void* especie) {
		...
		/*CREO CONEXION*/
		int socket_para_envio = conectar_como_productor(ip_broker,
				puerto_broker); // ESTA FUNCION ESTA EN DUDA, PORQUE LA COLA PRODUCTOR NO CREO QUE SE MANTENGA
		...
		*id_respuesta = enviar_mensaje(GET_POKEMON, a_enviar,
				socket_para_envio);
...
int suscribirse_a_queue(queue_name cola, char* ip_broker, char* puerto_broker) {
	...
	if (cola == APPEARED_POKEMON) {
		socket_cola = suscribirse_a_cola(APPEARED_POKEMON, ip_broker,
				puerto_broker);
		if (socket_cola == -1) {
		...
		socket_cola = suscribirse_a_cola(CAUGHT_POKEMON, ip_broker,
				puerto_broker);
		if (socket_cola == -1) {
		...
		socket_cola = suscribirse_a_cola(LOCALIZED_POKEMON, ip_broker,
				puerto_broker);
		if (socket_cola == -1) {
...
void recibirLocalized() { // no esta terminada
	...
	socket_localized = suscribirse_a_cola(LOCALIZED_POKEMON, ip_broker,
			puerto_broker);
	...
	printf(
			"Conexion con la cola de mensajes LOCALIZED_POKEMON exitosa. Esperando mensajes...");

	while (1) {
		mensaje_recibido_localized = recibir_mensaje(LOCALIZED_POKEMON,
				socket_localized);

		if (mensaje_recibido_localized != NULL) {
			if ((estaEnLaLista((mensaje_recibido_localized->nombre_pokemon),
					objetivos_globales))
					&& (!(estaEnLaLista(
							(mensaje_recibido_localized->nombre_pokemon),
							pokemons_recibidos)))) {
				agregarPokemonsRecibidosALista(pokemons_recibidos,
						mensaje_recibido_localized);
				t_list* todosLosEntrenadores =
						todosLosEntrenadoresAPlanificar();
				t_entrenador* entrenadorReady = entrenadorAReady(
						todosLosEntrenadores, pokemons_recibidos);

				bool esElMismo(void* entrenador) {
					return entrenador == entrenadorReady;
				}

				t_entrenador* removido = list_remove_by_condition(
						listaALaQuePertenece(entrenadorReady), esElMismo);
...
// ES UN ASCO
	...
	while (1) {
		...
		if (recv(socket_cliente, &cola, sizeof(queue_name), MSG_WAITALL)
				== -1) {
		...
		int bytes = recv(socket_cliente, &otra_cola, sizeof(queue_name),
				MSG_WAITALL);
		if (bytes != -1) {
			appeared_pokemon_msg* msg = recibir_mensaje(APPEARED_POKEMON,
					socket_cliente);
...
void algoritmoFifo(t_entrenador* entrenador)
{
	...
	int retardoCpu = config_get_int_value(config, "RETARDO_CICLO_CPU");
	uint32_t ciclos = 0;
	if (aMoverse!= NULL)
	{
		...
		int conexionExec = conectar_como_productor(ip_broker,puerto_broker);
		if (conexionExec != -1)
		{
